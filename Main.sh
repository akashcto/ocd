...
sudo docker run -d -i -t -p 0.0.0.0:8080:8080 wipro/ocd-jenkins:0.1.0 bash start.sh
sleep 60
...
sudo docker cp chef-server:/etc/chef-server/chef-validator.pem /etc/chef-server/
